...
export default class AiUsagePreferences extends ExtensionPreferences {
    ...
    _addCredentialRows(row, acc) {
        ...
        if (acc.provider === 'opencode-go') {
            row.add_row(this._entryRow(_('Service API Key'), c.apiKey || '', true, val =>
                this._updateAccount(acc.id, a => { a.credentials.apiKey = val; })));
            row.add_row(this._entryRow(_('Console URL (optional)'), c.consoleUrl || '', false, val =>
                this._updateAccount(acc.id, a => { a.credentials.consoleUrl = val; })));
        }
    ...
    _defaultCredentials(provider) {
        ...
        if (provider === 'opencode-go') return { apiKey: '', consoleUrl: '' };
        return {};
    }
